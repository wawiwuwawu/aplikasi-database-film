      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionDivider(),
            _buildStaffSection(convertToStaffModelList(movie.staffs)),
            _buildSectionDivider(),
  ...
  Widget _buildSeiyuSection(List<Seiyu> seiyus, List<Karakter> karakters) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        ...seiyus.map((seiyu) {
          final karakterRelasi =
              karakters
                  .where((k) => k.id == seiyu.seiyuMovie?.karakterId)
                  .toList();

          return ListTile(
            leading: CircleAvatar(
              backgroundImage:
                  (seiyu.profileUrl?.isNotEmpty == true)
                      ? CachedNetworkImageProvider(seiyu.profileUrl!)
                      : null,
              child:
                  (seiyu.profileUrl?.isEmpty ?? true)
                      ? const Icon(Icons.person)
                      : null,
            ),
